...
INSTALLED_APPS = [
    ...
    # AI Assistant Plugin - 使用完整的 AppConfig 路径
    "ai_assistant.config.AiAssistantConfig",
    # Excel Importer Plugin - Excel 导入导出插件
    "excel_importer.config.ExcelImporterConfig",
    # Row Author Tracker Plugin - 自动填写人追踪插件
...
PERMISSION_MANAGERS = [
    ...
    "member",
    "token",
    "access_control",  # 移到 role 之前，确保自定义权限检查优先
    "write_field_values",
    "role",
    "basic",
    "automation_workflow",
...
# 检查 access_control 插件是否安装(支持完整路径和简短名称)
if "access_control" not in INSTALLED_APPS and "access_control.apps.AccessControlConfig" not in INSTALLED_APPS:
    PERMISSION_MANAGERS.remove("access_control")
...
# 自定义插件列表（不在 BASEROW_BACKEND_PLUGIN_NAMES 中但需要加载设置的插件）
CUSTOM_PLUGINS_WITH_SETTINGS = [
    "access_control",
]

for plugin in [*BASEROW_BUILT_IN_PLUGINS, *BASEROW_BACKEND_PLUGIN_NAMES, *CUSTOM_PLUGINS_WITH_SETTINGS]:
    try:
        ...
        # 只在非预期的导入错误时打印
        if "No module named" not in str(e) or "config.settings.settings" not in str(e):
