<template>
  <Modal ref="modal" @hidden="onHidden">
    <h2 class="box__title">
      {{ $t('accessControl.field.fieldPermissions') }}
    </h2>
    <p v-if="field" class="margin-bottom-2">
      {{ field.name }}
    </p>

    <div v-if="loading" class="loading-overlay">
      <div class="loading"></div>
    </div>

    <div v-else class="field-permission-modal">
      <!-- 成员选择器 -->
      <div class="field-permission-modal__member-selector margin-bottom-2">
        <label class="control__label">
          {{ $t('accessControl.table.selectMember') }}
        </label>
        <Dropdown
          v-model="selectedMemberId"
          :disabled="saving"
          class="field-permission-modal__dropdown"
        >
          <DropdownItem
            v-for="member in collaboratingMembers"
            :key="member.user_id"
            :name="getMemberDisplayName(member)"
            :value="member.user_id"
          ></DropdownItem>
        </Dropdown>
        <p v-if="collaboratingMembers.length === 0" class="field-permission-modal__empty-hint">
          {{ $t('accessControl.table.noCollaboratingMembers') }}
        </p>
      </div>

      <!-- 当选择了成员时显示权限设置 -->
      <template v-if="selectedMember && collaboratingMembers.length > 0">
        <!-- 字段权限 -->
        <div class="field-permission-modal__section margin-bottom-2">
          <h3 class="field-permission-modal__section-title">
            {{ $t('accessControl.field.fieldPermissions') }}
          </h3>
          <p class="field-permission-modal__section-description">
            {{ $t('accessControl.field.fieldPermissionsDescription') }}
          </p>
          
          <div class="field-permission-modal__permission-options">
            <Radio
              :model-value="currentPermission.permission_level"
              value="editable"
              :disabled="saving"
              @input="updatePermissionLevel('editable')"
            >
              <div class="field-permission-modal__option-content">
                <span class="field-permission-modal__option-label">
                  {{ $t('accessControl.field.editable') }}
                </span>
                <span class="field-permission-modal__option-description">
                  {{ $t('accessControl.field.editableDescription') }}
                </span>
              </div>
            </Radio>
            
            <Radio
              :model-value="currentPermission.permission_level"
              value="read_only"
              :disabled="saving"
              @input="updatePermissionLevel('read_only')"
            >
              <div class="field-permission-modal__option-content">
                <span class="field-permission-modal__option-label">
                  {{ $t('accessControl.field.readOnly') }}
                </span>
                <span class="field-permission-modal__option-description">
                  {{ $t('accessControl.field.readOnlyDescription') }}
                </span>
              </div>
            </Radio>

            <Radio
              :model-value="currentPermission.permission_level"
              value="hidden"
              :disabled="saving"
              @input="updatePermissionLevel('hidden')"
            >
              <div class="field-permission-modal__option-content">
                <span class="field-permission-modal__option-label">
                  {{ $t('accessControl.field.hidden') }}
                </span>
                <span class="field-permission-modal__option-description">
                  {{ $t('accessControl.field.hiddenDescription') }}
                </span>
              </div>
            </Radio>
          </div>
        </div>
      </template>

      <!-- 错误提示 -->
      <div v-if="error" class="field-permission-modal__error margin-top-2">
        <Alert type="error">{{ error }}</Alert>
      </div>
    </div>

    <div class="actions">
      <div class="align-right">
        <Button
          type="secondary"
          size="large"
          :disabled="saving"
          @click="hide"
        >
          {{ $t('action.close') }}
        </Button>
      </div>
    </div>
  </Modal>
</template>

<script>
import modal from '@baserow/modules/core/mixins/modal'
import { notifyIf } from '@baserow/modules/core/utils/error'
import fieldPermissionsService from '@access-control/services/fieldPermissions'
import databaseCollaborationService from '@access-control/services/databaseCollaboration'

export default {
  name: 'FieldPermissionModal',
  mixins: [modal],
  props: {
    field: {
      type: Object,
      default: null,
    },
    table: {
      type: Object,
      default: null,
    },
    database: {
      type: Object,
      default: null,
    },
  },
  data() {
    return {
      loading: false,
      saving: false,
      error: null,
      selectedMemberId: null,
      permissions: [],
      collaboratingMembers: [],
      currentPermission: {
        id: null,
        permission_level: 'editable',
      },
    }
  },
  computed: {
    fieldService() {
      return fieldPermissionsService(this.$client)
    },
    collaborationService() {
      return databaseCollaborationService(this.$client)
    },
    selectedMember() {
      if (!this.selectedMemberId) return null
      return this.collaboratingMembers.find(m => m.user_id === this.selectedMemberId)
    },
    /**
     * 检查当前用户是否是工作空间管理员
     * 只有管理员才能管理字段权限
     */
    isWorkspaceAdmin() {
      const workspaceId = this.database?.workspace?.id || this.database?.workspace_id
      if (!workspaceId) return false
      const workspace = this.$store.getters['workspace/get'](workspaceId)
      return workspace && workspace.permissions === 'ADMIN'
    },
  },
  watch: {
    selectedMemberId: {
      handler(newVal) {
        if (newVal) {
          this.loadMemberPermission()
        }
      },
    },
  },
  methods: {
    async show() {
      this.getRootModal().show()
      await this.loadData()
    },
    hide() {
      this.getRootModal().hide()
    },
    onHidden() {
      this.reset()
    },
    reset() {
      this.selectedMemberId = null
      this.permissions = []
      this.collaboratingMembers = []
      this.currentPermission = {
        id: null,
        permission_level: 'editable',
      }
      this.error = null
    },
    async loadData() {
      if (!this.field || !this.table || !this.database) return

      // 只有管理员才能加载字段权限数据
      if (!this.isWorkspaceAdmin) {
        this.loading = false
        return
      }

      this.loading = true
      this.error = null

      try {
        // 获取工作空间 ID
        const workspaceId = this.database?.workspace?.id || this.database?.workspace_id
        
        // 并行加载字段权限、数据库协作设置和工作空间成员
        const [permissionsResponse, collaborationsResponse, membersResponse] = await Promise.all([
          this.fieldService.getPermissions(this.field.id),
          this.collaborationService.getCollaborations(this.database.id),
          this.loadWorkspaceMembers(workspaceId),
        ])

        this.permissions = permissionsResponse.data || []
        
        // 从协作设置中获取有该表访问权限的成员
        const collaborations = collaborationsResponse.data || []
        const allMembers = membersResponse || []
        
        // 创建用户 ID 到成员信息的映射
        const memberMap = {}
        allMembers.forEach(m => {
          memberMap[m.user_id] = m
        })
        
        this.collaboratingMembers = collaborations
          .filter(c => {
            // 检查成员是否有该表的访问权限
            const accessibleTables = c.accessible_tables || []
            return accessibleTables.includes(this.table.id)
          })
          .map(c => ({
            user_id: c.user.id,
            name: c.user.first_name || c.user.username || c.user.email,
            email: c.user.email,
            user: c.user,
          }))

        // 如果有成员,默认选择第一个
        if (this.collaboratingMembers.length > 0) {
          this.selectedMemberId = this.collaboratingMembers[0].user_id
        }
      } catch (error) {
        this.error = error.message || this.$t('accessControl.errors.failedToLoadData')
        notifyIf(error, 'field')
      } finally {
        this.loading = false
      }
    },
    async loadWorkspaceMembers(workspaceId) {
      // 从当前工作空间获取成员
      try {
        let users = []
        
        // 首先尝试从 store 获取
        const workspace = this.$store.getters['workspace/get'](workspaceId)
        if (workspace && workspace.users && workspace.users.length > 0) {
          users = workspace.users
        } else {
          // 如果 store 中没有用户数据，从 API 获取
          const { data } = await this.$client.get(`/workspaces/users/workspace/${workspaceId}/`)
          users = data || []
        }
        
        if (users.length > 0) {
          // 过滤掉管理员(管理员默认有所有权限)
          return users
            .filter(m => m.permissions !== 'ADMIN')
            .map(m => ({
              user_id: m.user_id || m.id,
              id: m.id || m.user_id,
              name: m.name || m.first_name,
              first_name: m.first_name,
              email: m.email,
              permissions: m.permissions,
            }))
        }
        
        return []
      } catch (error) {
        console.error('加载工作空间成员失败:', error)
        return []
      }
    },
    loadMemberPermission() {
      // 查找当前成员的权限设置
      const permission = this.permissions.find(
        p => p.user && p.user.id === this.selectedMemberId
      )

      if (permission) {
        this.currentPermission = {
          id: permission.id,
          permission_level: permission.permission_level || 'editable',
        }
      } else {
        // 重置为默认值(可编辑)
        this.currentPermission = {
          id: null,
          permission_level: 'editable',
        }
      }
    },
    getMemberDisplayName(member) {
      if (member.name) return member.name
      if (member.user && member.user.first_name) return member.user.first_name
      if (member.email) return member.email
      return `User ${member.user_id}`
    },
    async updatePermissionLevel(level) {
      await this.savePermission({ permission_level: level })
    },
    async savePermission(updates) {
      if (!this.selectedMemberId) return

      this.saving = true
      this.error = null

      try {
        const newData = {
          ...this.currentPermission,
          ...updates,
        }

        let result
        if (this.currentPermission.id) {
          // 更新现有权限设置
          const { data } = await this.fieldService.updatePermission(
            this.field.id,
            this.currentPermission.id,
            updates
          )
          result = data
        } else {
          // 创建新的权限设置
          const { data } = await this.fieldService.createPermission(this.field.id, {
            user_id: this.selectedMemberId,
            permission_level: newData.permission_level,
          })
          result = data
          // 添加到权限列表
          this.permissions.push(result)
        }

        // 更新本地状态
        this.currentPermission = {
          id: result.id,
          permission_level: result.permission_level || 'editable',
        }

        // 更新权限列表中的记录
        const index = this.permissions.findIndex(p => p.id === result.id)
        if (index !== -1) {
          this.$set(this.permissions, index, result)
        }

        this.$emit('permission-updated', {
          fieldId: this.field.id,
          userId: this.selectedMemberId,
          permission: result,
        })
      } catch (error) {
        this.error = error.message || this.$t('accessControl.errors.failedToSavePermission')
        notifyIf(error, 'field')
      } finally {
        this.saving = false
      }
    },
  },
}
</script>
